/* Updated: 2026-07-14 23:30 UTC | v2.1.1 | Fix NCHW pixel order + try-catch */
...
class YoloDetector(private val context: Context) {
    ...
    fun detect(bitmap: Bitmap): List<Detection> {
        ...
        for (i in pixels.indices) {
            inputBytes.putFloat(((pixels[i] shr 16) and 0xFF).toFloat() / 255f)
        }
        for (i in pixels.indices) {
            inputBytes.putFloat(((pixels[i] shr 8) and 0xFF).toFloat() / 255f)
        }
        for (i in pixels.indices) {
            inputBytes.putFloat((pixels[i] and 0xFF).toFloat() / 255f)
        }
        inputBytes.rewind()
