...
if (-not $SkipModel) {
    if (-not (Test-Path $TFLITE)) {
        ...
        if (Test-Path $TFLITE) {
            ...
            Write-Host "  WARNING: Generate dummy model gagal, app akan error saat predict" -ForegroundColor Yellow
        }
    } else {
...
# ── 4. Install Python deps (skip jika sudah) ─────────────
Write-Host ""
