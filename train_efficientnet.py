"""
TBS Kelapa Sawit - EfficientNetB0 Training v2.0.0
==================================================
Two-phase training: head (5 epoch) → fine-tune (5 epoch)
Target: 80-85% val accuracy on 5-class TBS classification
...
Output:
  - backend/model_output/model_tbs_effnet_final.keras
  - backend/model_output/model_tbs_effnet.tflite
  - backend/model_output/labels.txt
  - backend/model_output/model_info.json

Usage:
  python train_efficientnet.py

Google Colab: Upload repo, enable GPU, run this script.
Local VSCode: pip install -r requirements.txt && python train_efficientnet.py
"""
...
def build_model(num_classes=5):
    print("[4/7] Building EfficientNetB0 model...")
...
def save_model(model, results):
    ...
    keras_path = os.path.join(OUTPUT_DIR, "model_tbs_effnet_final.keras")
    ...
    tflite_path = os.path.join(OUTPUT_DIR, "model_tbs_effnet.tflite")
    ...
    meta = {
        ...
        "architecture": "EfficientNetB0 + fine-tune",
...
def main():
    print("="*60)
    print("  TBS KELAPA SAWIT - EfficientNetB0 TRAINING v2.0.0")
    print("="*60)
    ...
    print("="*60)
    print("\nNext steps:")
    print("  1. Convert to TF.js: python convert_effnet_tfjs.py")
